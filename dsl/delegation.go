...
func BuildTeamPrompt(system string, team []string, agentDescriptions map[string]string, blackboardEnabled bool) string {
	...
	teamSection += "\nPost status updates and results to your team channel using `post_to_channel` so the user can see progress."
	teamSection += "\nIMPORTANT: Channels are just a log — other agents CANNOT see channel messages. To assign work, you MUST use `delegate`. Posting a plan to a channel does NOT make anything happen."
	teamSection += "\n\n## Reporting to Hermes"
